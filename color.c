...
SDL_Texture *LoadHSVTexture(SDL_Renderer *rend) {
	...
	unsigned char* data = stbi_load_from_memory(HSV_PNG, HSV_PNG_LEN, &width, &height, &orig_format, req_format);
	if(data == NULL) {
...
SDL_bool pickColorHSV(SDL_Renderer *rend, SDL_Texture *font, SDL_Color *ret) {
	SDL_Rect xhair[4] = {{0, 0, XHAIRW, XHAIRH}, {0, 0, XHAIRW, XHAIRH}, {0, 0, XHAIRH, XHAIRW}, {0, 0, XHAIRH, XHAIRW}};
	...
	SDL_Rect cancelButRect = {HSVX+okButRect.w+1, okButRect.y, 8*LETWIDTH, LETHEIGHT+2};
	SDL_Rect satThumb = {SLIDERWIDTH+SLIDEROFFSET, HSVY+130+(2*SLIDERTHUMBSIZE), SLIDERTHUMBSIZE, SLIDERTHUMBSIZE};
	SDL_Rect aThumb = {SLIDERWIDTH+SLIDEROFFSET, HSVY+130+(4*SLIDERTHUMBSIZE), SLIDERTHUMBSIZE, SLIDERTHUMBSIZE};
	...
	for(;;){
		...
		drawText(rend, "Value:", font, 20*LETWIDTH, textinfoAnchor+LETHEIGHT+LETHEIGHT);
		drawText(rend, vString, font, 32*LETWIDTH, textinfoAnchor+LETHEIGHT+LETHEIGHT);
		SDL_RenderPresent(rend);
