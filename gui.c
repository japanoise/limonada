...
GtkWidget *create_filechooser_dialog(char *init_path, GtkFileChooserAction action)
{
	...
	case GTK_FILE_CHOOSER_ACTION_OPEN:
		...
		g_signal_connect(wdg, "update-preview", G_CALLBACK(update_preview_cb), preview);
		break;
	}
	return wdg;
}
...
int compareDirent(const void *l, const void *r)
{
	...
	if ((*left)->d_type == DT_DIR && (*right)->d_type != DT_DIR)
		return LEFT_FIRST;
	else if ((*left)->d_type != DT_DIR && (*right)->d_type == DT_DIR)
		return RIGHT_FIRST;
	else
...
char *fileBrowse(SDL_Renderer * rend, SDL_Texture * font, char *dir, enum fileFlags flags)
{
	...
	while (!done) {
		...
		if (flags & fileFlag_NewFiles) {
			...
			if (okRec.x < mousex && mousex < okRec.x + okRec.w && okRec.y < mousey
			    && mousey < okRec.y + okRec.h)
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_MOUSEBUTTONUP:
				...
				if (event.button.button == SDL_BUTTON_LEFT) {
					...
					} else if (flags & fileFlag_NewFiles
						   && (okRec.x < mousex
						       && mousex < okRec.x + okRec.w
						       && okRec.y < mousey
						       && mousey < okRec.y + okRec.h)) {
